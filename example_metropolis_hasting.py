...
log_prior_mean = lambda mu: stats.norm.logpdf(mu, loc=0, scale=0.5)
log_prior_sigma = lambda sigma: stats.norm.logpdf(sigma, loc=2, scale=0.5)

lupost = lambda param: np.sum(stats.norm.logpdf(x, loc=param[0], scale=np.exp(param[1]))) \
                       * log_prior_mean(param[0]) \
                       * log_prior_sigma(np.exp(param[1]))
